...
We will see a quick setup to configure the ``gitea`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``gitea`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``gitea`` formulas under ``/srv/salt/gitea/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/gitea/parameters/dns:domain/
    ln -s dns:domain /srv/salt/gitea/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/gitea/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/gitea/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``gitea`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``gitea`` formulas under ``/srv/salt/gitea/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/gitea/parameters/roles
...
We create a configuration for the role ``gitea/server`` in ``/srv/salt/gitea/parameters/roles/gitea/server.yaml``:
...
We create another configuration for the role ``gitea/client`` in ``/srv/salt/gitea/parameters/roles/gitea/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles gitea/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles gitea/client
...
.. code-block:: console
    ...
    mv /srv/salt/gitea/parameters/dns:domain/  '/srv/salt/gitea/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-gitea-as-json:
      test.nop:
        - name: {{ gitea | json }}
...
Here is an example based on `template-formula/gitea/config/file.sls`_:
...
This ``sls`` file expose a ``gitea`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ gitea['winner'] }}
...
.. _template-formula/gitea/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/gitea/config/file.sls
